...
def processExtreme(read_path, save_path, newShape = np.array((200,200)), CropOrPad = 0 , cropShape = np.array([0,0,0,0])):
    
    '''
        read_path should be the path to the raw extreme picture finder folder containing images for all the classes.
        The script iterates on all its subfolders recursively and saves resized images to the saved_path
    '''
    
    classFolders = [name for name in os.listdir(read_path) if os.path.isdir(os.path.join(read_path, name))]
    ...
    for classFolder in classFolders:
        ...
        # Avoid the hidden file '.DS_Store'
        try:
            classFolder.remove('.DS_Store')
        except:
            pass
        
        # Resize images not in a subfolder
        files = [name for name in os.listdir(read_path + "\\" + classFolder) if os.path.isfile(os.path.join(read_path + "\\" + classFolder, name))]
        for filename in files:
            
            alreadyExistAtDestination = os.path.isfile(save_path + "\\" + classFolder + "\\" + filename)
            isSupported = True
            canResize = True
            
            if not alreadyExistAtDestination:
                # Read and scale the image if the image is supported
                try: 
                    img = Image.open(read_path + "\\" + classFolder + "\\" + filename)
                except:
                    countFails += 1
                    print("[" + classFolder + "][FAIL {0}] This image is not supported and is skipped: {1}".format(countFails, read_path + "\\" + classFolder + "\\" + filename))
                    isSupported = False
                    
                if isSupported:
                        
                    try:
                        img = imread(read_path + "\\" + classFolder + "\\" + filename)
                        (newImg , size) = reSizePad(img , newShape , CropOrPad , cropShape)
                    except:
                        countFails += 1
                        print("[" + classFolder + "][FAIL {0}] imread or reSizePad failed for this image: {1}".format(countFails, read_path + "\\" + classFolder + "\\" + filename))
                        canResize = False
                    
                    if canResize:
                        # Create folders if they do not exist
                        if not exists(save_path + "\\" + classFolder):
                            makedirs(save_path + "\\" + classFolder)
                        
                        # Save the scaled image
                        if (".jpg" in filename or ".JPG" in filename):
                            try:
                                imsave(save_path + "\\" + classFolder + "\\" + filename , newImg)
                                countSuccesses += 1
                                print("[" + classFolder + "][SUCCESS {0}] This image was resized and saved: {1}".format(countSuccesses, save_path + "\\" + classFolder + "\\" + filename))
                            except:
                                countFails += 1
                                print("[" + classFolder + "][FAIL {0}] This image failed to save for some reason: {1}".format(countFails, save_path + "\\" + classFolder + "\\" + filename))
                        else:
                            countFails += 1
                            print("[" + classFolder + "][FAIL {0}] This image was a non-jpg: {1}".format(countFails, save_path + "\\" + classFolder + "\\" + filename))
        
            else:
                countSuccesses += 1
                countAlreadyExist += 1
                print("[" + classFolder + "][SKIPPED {0}] This image has already been resized: {1}".format(countAlreadyExist, save_path + "\\" + classFolder + "\\" + filename))
                
        # Resize the images in the subfolders
        subfolders = [name for name in os.listdir(read_path + "\\" + classFolder) if os.path.isdir(os.path.join(read_path + "\\" + classFolder, name))]
        for subfolder in subfolders:
            ...
            # Resize images in this subfolder
            files = [name for name in os.listdir(read_path + "\\" + classFolder + "\\" + subfolder) if os.path.isfile(os.path.join(read_path + "\\" + classFolder + "\\" + subfolder, name))]
            for filename in files:
                
                # Create new name using subfolder name to avoid name conflicts in the destination folder
                newFilename = ".".join(filename.split(".")[0:-1]) + "__" + subfolder.replace(".","DOT") + "." + filename.split(".")[-1]
                
                alreadyExistAtDestination = os.path.isfile(save_path + "\\" + classFolder + "\\" + newFilename)
                isSupported = True
                canResize = True
                ...
                if not alreadyExistAtDestination:
                    # Read and scale the image if the image is supported
                    try: 
                        img = Image.open(read_path + "\\" + classFolder + "\\" + subfolder + "\\" + filename)
                    except:
                        countFails += 1
                        print("[" + classFolder + "][FAIL {0}] This subfoldered image is not supported and is skipped: {1}".format(countFails, read_path + "\\" + classFolder + "\\" + subfolder + "\\" + filename))
                        isSupported = False
                    
                    if isSupported:
                        try:   
                            img = imread(read_path + "\\" + classFolder + "\\" + subfolder + "\\" + filename)
                            (newImg , size) = reSizePad(img , newShape , CropOrPad , cropShape)
                        except:
                            countFails += 1
                            print("[" + classFolder + "][FAIL {0}] imread or reSizePad failed for this subfoldered image: {1}".format(countFails, read_path + "\\" + classFolder + "\\" + subfolder + "\\" + filename))
                            canResize = False
                        ...
                        if canResize:
                            ...
                            if (".jpg" in filename or ".JPG" in filename):
                                try:
                                    imsave(save_path + "\\" + classFolder + "\\" + newFilename , newImg)
                                    countSuccesses += 1
                                    print("[" + classFolder + "][SUCCESS {0}] This subfoldered image was resized and saved: {1}".format(countSuccesses, save_path + "\\" + classFolder + "\\" + newFilename))
                                except:
                                    countFails += 1
                                    print("[" + classFolder + "][FAIL {0}] This subfoldered image failed to save for some reason: {1}".format(countFails, save_path + "\\" + classFolder + "\\" + newFilename))
                            else:
                                countFails += 1
                                print("[" + classFolder + "][FAIL {0}] This subfoldered image was a non-jpg: {1}".format(countFails, save_path + "\\" + classFolder + "\\" + newFilename))

                else:
                    countSuccesses += 1
                    countAlreadyExist += 1
                    print("[" + classFolder + "][SKIPPED {0}] This subfoldered image has already been resized: {1}".format(countAlreadyExist, save_path + "\\" + classFolder + "\\" + newFilename))
                               
    return 0

def partition_extreme(test_ratio, dataPath):
    
    '''
        The function takes a train/test ratio and the dataparh and partitions the images in a training and testing set folder
    '''
    
    classFolders = [name for name in os.listdir(dataPath) if os.path.isdir(os.path.join(dataPath, name))]
    ...
    try:
        classFolders.remove('training_images')
    except:
        pass
    
    try:
        classFolders.remove('testing_images')
    except:
        pass
        
    trianPath = dataPath + "\\training_images"
    testPath = dataPath + "\\testing_images"
        
    # Create folders if they do not exist
    if not exists(trianPath):
        makedirs(trianPath)
        
    if not exists(testPath):
        makedirs(testPath)
        
    numberOfImagesPerClass = []
    for classFolder in classFolders:
        if not exists(testPath + "\\" + classFolder):
            makedirs(testPath + "\\" + classFolder)
            
        if not exists(trianPath + "\\" + classFolder):
            makedirs(trianPath + "\\" + classFolder)
        ...
        files = [name for name in os.listdir(dataPath + "\\" + classFolder) if os.path.isfile(os.path.join(dataPath + "\\" + classFolder, name))]
        numberOfImagesPerClass.append(len(files))
        
    print(classFolders)
    print(numberOfImagesPerClass)
    totalToPick = min(numberOfImagesPerClass)
    testToPick = math.floor(test_ratio*totalToPick)
    trainToPick = totalToPick - testToPick
    print(totalToPick)
    print(testToPick)
    print(trainToPick)
    print(testToPick+trainToPick)
        
    for classFolder in classFolders:
        files = [name for name in os.listdir(dataPath + "\\" + classFolder) if os.path.isfile(os.path.join(dataPath + "\\" + classFolder, name))]            
        # Pick random test image from folder
        testFiles = random.sample(files, testToPick)
        for filename in testFiles:
            # Move the image to testing images and delete the original
            try:
                shutil.move(dataPath + "\\" + classFolder + "\\" + filename, testPath + "\\" + classFolder + "\\" + filename)
                files.remove(filename)
                print("Successfully moved testing image: {0}".format(dataPath + "\\" + classFolder + "\\" + filename))
            except:
                print("Failed to move testing image: {0}".format(dataPath + "\\" + classFolder + "\\" + filename))
                print(dataPath + "\\" + classFolder + "\\" + filename)
                print(testPath + "\\" + classFolder + "\\" + filename)
            
        # Pick random training image from folder
        trainingFiles = random.sample(files, trainToPick)
        for filename in trainingFiles:
            # Move the image to training images and delete the original
            try:
                shutil.move(dataPath + "\\" + classFolder + "\\" + filename, trianPath + "\\" + classFolder + "\\" + filename)
                print("Successfully moved training image: {0}".format(dataPath + "\\" + classFolder + "\\" + filename))
            except:
                print("Failed to move training image: {0}".format(dataPath + "\\" + classFolder + "\\" + filename))
                print(dataPath + "\\" + classFolder + "\\" + filename)
                print(trianPath + "\\" + classFolder + "\\" + filename)
...
nTypes = 1 # Number of distinct food types to download
nEacgFood = 10000 # Number of images to download from each category
...
#read_path = 'C:\\food-training-images-database\\' + 'data\\' + 'food images\\' + 'google\\' + timestamp + '\\DL'
read_path = 'C:\\ExtremePictureFinder\\Raw'
save_path = 'C:\\ExtremePictureFinder\\Processed'

#resizeImageFolder(read_path, save_path = read_path , newShape = newShape , CropOrPad = 0)
processExtreme(read_path, save_path, newShape, CropOrPad, cropShape)

#partition_extreme(trainRatio, save_path)
